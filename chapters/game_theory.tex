...
\fb{Why are here pre pages everywhere? Don’t start at a new page.

As discussed before, it would be interessting to add a section 3.3 where you survey GT approafhes in Smart grids on a high level before diving into the two main topics in 4 and 5. The DEveloping Smart Grids” part is not covered in Section 3 so far. At least not in an obvious way. I would use 3.2. to intorduce these games more compactly and then 3.3. to explain how these models are used in Smart grids and then transit to 4. Seems more natural to me.
